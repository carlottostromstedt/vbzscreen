...
def fetch_and_display_connections(epd, draw, counter, weather_counter, temperature, weather_description, temperature_max, temperature_min):
  ...
  try:
    ...
    text_draw.text((220, 6), str(current_hour), font=font_time, fill=0) 
    ...
    text_draw.text((220, 30), str(current_minutes), font=font_time, fill=0)
    ...
    text_draw.text((220, 60), f"{int(temperature)}°C" , font=font_weather, fill=0)
    text_draw.text((220, 80), f"{weather_description}" , font=font_weather, fill=0)
    text_draw.text((220, 100), f"H: {temperature_max}°C L: {int(temperature_min)}°C", font=font_weather, fill=0)
